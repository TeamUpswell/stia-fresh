...
export default function RoomChecklist() {
  ...
  const handleCompleteAll = async () => {
    try {
      ...
      const { error } = await supabase
        ...
        .in('id', tasks.map(task => task.visit_task_id));
  ...
  const loadTasksForVisit = async () => {
    try {
      if (!property || !currentVisit) return;
  ...
  return (
    <AuthenticatedLayout>
      <div className="container mx-auto py-8 px-4">
        <div className="mb-6">
          <Link href="/cleaning/checklist" className="flex items-center text-blue-600 hover:text-blue-800">
        ...
        <div className="flex items-center justify-between mb-6">
          <div className="flex items-center">
            ...
            <h1 className="text-2xl font-semibold">{roomNames[roomId as keyof typeof roomNames] || roomId}</h1>
          </div>
          
          <div className="flex space-x-2">
            <Link href={`/cleaning/history${currentVisit ? `?visit=${currentVisit.id}` : ''}`}>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            {tasks.length === 0 ? (
              <div className="py-12 text-center">
                <p className="text-gray-500 mb-4">No tasks defined for this room yet.</p>
            ...
            {tasks.length > 0 && (user?.roles?.includes('owner') || user?.roles?.includes('manager')) && (
