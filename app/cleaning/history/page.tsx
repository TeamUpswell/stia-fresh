...
export default function CleaningHistory() {
  ...
  const [selectedVisit, setSelectedVisit] = useState<string | null>(
    ...
    async function loadData() {
      try {
        ...
        if (currentProperty?.id) {
          await fetchVisits(currentProperty.id);
        }
      } catch (error) {
                        ...
                        {selectedVisit === visit.id && visitDetails && (
                          <div className="mt-2">
                            <div className="flex justify-between text-xs text-gray-600 mb-1">
                              ...
                              <span>
                                {getVisitStats(visit)?.percentage || 0}%
                              </span>
                            </div>
                ...
                <div className="bg-white rounded-lg shadow-md overflow-hidden">
                  <div className="p-4 bg-blue-50 border-b border-blue-100 flex justify-between">
                    ...
                    <div className="text-sm">
                      {getVisitStats(visits.find((v) => v.id === selectedVisit))
                        ?.completed || 0}{" "}
                      /
                      {getVisitStats(visits.find((v) => v.id === selectedVisit))
                        ?.total || 0}{" "}
                  ...
                  <div className="p-4">
                    {Object.keys(visitDetails.roomTasks).length === 0 ? (
                      ...
                      Object.entries(visitDetails.roomTasks).map(
                        ([room, tasks]) => {
